...
{
	string configstr = "kingdom_vars.cfg";
	ConfigFile cfg;
...
shared class SandboxSpawns : RespawnSystem
{
	...
	bool SetMaterials(CBlob@ blob,  const string &in name, const int quantity)
	{
		...
		//already got them?
		if (inv.isInInventory(name, quantity)) return false;
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (canSpawnPlayer(p_info))
		{
			...
			//todo: don't hard set builder. respawn with appropriate class
			p_info.blob_name = "builder"; //hard-set the respawn blob

			//todo: set p_info.team to the correct team so they respawn in the correct team
	...
	void AddPlayerToSpawn(CPlayer@ player)
	{
		...
		RemovePlayerFromSpawn(player);
		if (player.getTeamNum() == core.rules.getSpectatorTeamNum()) return;
